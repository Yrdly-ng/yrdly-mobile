...
function MarketplaceDetailContent() {
  ...
  if (!post) {
    return (
      <SafeAreaView style={[stylesheet.centerContainer, { backgroundColor: theme.colors.DARK }]}>
        <Text
          style={[stylesheet.errorText, { color: theme.colors.TEXT_PRIMARY, fontFamily: 'Outfit' }]}
        >
          Item not found
        </Text>
        <TouchableOpacity
          style={[
            stylesheet.backBtnWrapper,
            {
  ...
  const parsedImageUrls = Array.isArray(post.image_urls)
    ...
    : typeof post.image_urls === 'string'
      ...
      : [];
  const imageUrls =
    parsedImageUrls.length > 0 ? parsedImageUrls : post.image_url ? [post.image_url] : [];
  const mediaItems = [];
                  ...
                  return (
                    <View key={i} style={stylesheet.mainImageContainer}>
                      {media.type === 'video' ? (
                        <MarketVideo
                          url={media.url}
                          shouldPlay={activeScrollIndex === i && isFocused}
                        />
                      ) : (
                        <TouchableOpacity
                          activeOpacity={0.9}
              ...
              <View style={stylesheet.galleryControls}>
                ...
                {mediaItems.length > 1 && (
                  <View style={stylesheet.counterBadge}>
                    <Text style={stylesheet.counterText}>
                      {activeScrollIndex + 1}/{mediaItems.length}
                    </Text>
                  </View>
                )}
              </View>
          ...
          <View>
            ...
            <Text
              style={{
                ...
                fontWeight: '800',
                fontSize: 28,
                color: post.is_sold ? theme.colors.LABEL : theme.colors.G,
              }}
            >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4 }}>
              ...
              <Text
                style={{ fontFamily: 'Inter-Regular', fontSize: 13, color: theme.colors.LABEL }}
              >
                {post.lga ? `${post.lga}, ` : ''}
                {post.state || 'Location'}
              </Text>
          ...
          {isOwner ? (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                >
                  Use the buttons below to manage your listing
                </Text>
              </View>
            </View>
          ) : (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <TouchableOpacity onPress={() => router.push(`/profile/${post.user_id}` as any)}>
                  ...
                  >
                    {postUser?.name || post.author_name || 'Unknown Seller'}
                  </Text>
                </TouchableOpacity>
      ...
      >
        {isOwner ? (
          <>
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                height: 52,
                borderRadius: 16,
                backgroundColor: post.is_sold ? 'rgba(255,92,92,0.1)' : 'rgba(130,219,126,0.1)',
                borderWidth: 1,
                borderColor: post.is_sold ? 'rgba(255,92,92,0.3)' : 'rgba(130,219,126,0.3)',
                alignItems: 'center',
                justifyContent: 'center',
              }}
            >
              <Text
                style={{
                  color: post.is_sold ? '#FF5C5C' : theme.colors.G,
                  fontFamily: 'Outfit-Bold',
                  fontWeight: '700',
                  fontSize: 15,
                }}
          ...
          <>
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { id: post.id, type: 'marketplace' },
                })
              }
              style={{
                flex: 1,
                height: 52,
                borderRadius: 16,
                backgroundColor: post.is_sold ? theme.colors.SURFACE : theme.colors.G,
                alignItems: 'center',
                justifyContent: 'center',
              }}
            >
              <Text
                style={{
                  color: post.is_sold ? theme.colors.LABEL : '#000',
                  fontFamily: 'Outfit-Bold',
                  fontWeight: '700',
                  fontSize: 16,
                }}
              >
                {post.is_sold ? 'Item Sold' : 'Buy Now'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <Text style={{ color: theme.colors.MUTED, fontFamily: 'Inter-Medium', fontSize: 14 }}>
                Message Seller
              </Text>
            </TouchableOpacity>
          </>
