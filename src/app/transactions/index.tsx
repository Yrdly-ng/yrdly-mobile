...
type EscrowStatus =
  'pending' | 'paid' | 'shipped' | 'delivered' | 'completed' | 'disputed' | 'cancelled' | 'failed';
...
export default function TransactionsScreen() {
      ...
      try {
        ...
        const normalised = (await Promise.all(
          (data ?? []).map(async (tx: any) => {
            ...
            const catalogItem = Array.isArray(tx.catalog_item)
              ? tx.catalog_item[0]
              : tx.catalog_item;
  ...
  const filteredData = transactions.filter((tx) => {
    ...
    if (filter === 'active')
      return tx.status === 'pending' || tx.status === 'paid' || tx.status === 'shipped';
  ...
  const renderItem = ({ item: tx }: { item: Transaction }) => {
    ...
    const imagesArr = Array.isArray(tx.item?.images)
      ? tx.item?.images
    ...
    return (
      ...
      >
        ...
        <View style={s.rowMid}>
          ...
          <Text style={s.rowParty} numberOfLines={1}>
    ...
    <SafeAreaView style={s.root} edges={['top', 'bottom']}>
        ...
        <View style={s.roleTabsInner}>
          {(['purchases', 'sales'] as Tab[]).map((t) => {
            ...
            return (
              ...
              >
                <Text
                  style={[s.roleTabTxt, { color: active ? theme.colors.DARK : theme.colors.MUTED }]}
                >
                  {t}
                </Text>
              </TouchableOpacity>
      ...
      <View style={s.filtersWrap}>
        <ScrollView
          ...
          contentContainerStyle={s.filtersPad}
        >
          {FILTERS.map((f) => {
